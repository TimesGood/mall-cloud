...
import com.mall.admin.domain.dto.PmsProductCategoryParam;
...
public class PmsProductCategoryController {
    ...
    @ResponseBody
    public R<Page<PmsProductCategory>> getList(@PathVariable Long parentId,
                                                                @RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize,
                                                                @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum) {
        List<PmsProductCategory> productCategoryList = productCategoryService.getList(parentId, pageSize, pageNum);
        return R.ok(Page.restPage(productCategoryList));
    }

    @ApiOperation("根据id获取商品分类")
    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    @ResponseBody
    public R<PmsProductCategory> getItem(@PathVariable Long id) {
    ...
    @ResponseBody
    public R<List<PmsProductCategoryWithChildrenItem>> listWithChildren() {
